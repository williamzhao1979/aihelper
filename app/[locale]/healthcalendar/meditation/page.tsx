...
import { ArrowLeft, Upload, X, FileText, Image, File, Users, Brain } from "lucide-react"
...
import type { FileAttachment } from '@/hooks/use-meditation-records'
...
export default function MeditationPage() {
  ...
  console.log("[MeditationPage] selectedUser?.uniqueOwnerId:", selectedUser?.uniqueOwnerId)
  console.log("[MeditationPage] selectedUser:", selectedUser)
  console.log("[MeditationPage] currentUser?.uniqueOwnerId:", currentUser?.uniqueOwnerId)

  const handleUserSelectionChange = (user: UserProfile) => {
    console.log('[MeditationPage] User selection changed to:', user)
    // 用户选择变化会通过全局状态自动同步
  }

  // meditationRecordsApi 必须在 selectedUser 声明后
  const meditationRecordsApi = useMeditationRecords(
    selectedUser?.ownerId || '',
    selectedUser?.uniqueOwnerId || ''
  )

  // 使用与view页面完全一致的映射逻辑
  const mappedMeditationRecords = useMemo(() => {
    console.log('[mappedMeditationRecords] useMemo triggered')
    console.log('[mappedMeditationRecords] meditationRecords length:', meditationRecordsApi.records.length)
    console.log('[mappedMeditationRecords] selectedUser:', selectedUser)
    console.log('[mappedMeditationRecords] mapping records, meditationRecords:', meditationRecordsApi.records)
    return meditationRecordsApi.records.map((r) => ({
      id: r.id,
      recordId: r.id,
  ...
  }, [meditationRecordsApi.records, selectedUser])

  // 强制获取最新数据 - 每次进入meditation页面时都强制刷新云端数据（与view页面保持一致）
  useEffect(() => {
    if (!selectedUser?.uniqueOwnerId) return
    console.log('[useEffect] Meditation页面强制云端刷新触发. selectedUser:', selectedUser)
    
    const doForceRefresh = async () => {
      try {
        console.log('[useEffect] Meditation页面开始强制云端刷新，用户:', selectedUser?.uniqueOwnerId)
        // 使用forceRefresh确保清除所有缓存并获取最新数据
        await meditationRecordsApi.forceRefresh()
        console.log('[useEffect] Meditation页面强制云端刷新完成')
      } catch (err) {
        console.error('[useEffect] Meditation页面强制云端刷新失败，尝试syncFromCloud:', err)
        try {
          await meditationRecordsApi.syncFromCloud()
          console.log('[useEffect] Meditation页面syncFromCloud完成')
        } catch (syncErr) {
          console.error('[useEffect] Meditation页面syncFromCloud也失败:', syncErr)
        }
      }
    }
    doForceRefresh()
    ...
    if (editId) {
      ...
      const loadEditRecord = async () => {
        try {
          ...
          // 等待records数据加载完成
          if (mappedMeditationRecords.length === 0) {
            // 如果records还没加载，等待一下
            setTimeout(loadEditRecord, 100)
            return
          }
          
          const recordToEdit = mappedMeditationRecords.find(r => r.id === editId)
  ...
  }, [searchParams, mappedMeditationRecords, toast])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const uploadedFile of uploadedFiles) {
        try {
          ...
          if (uploadedFile.file) {
            fileUrl = await meditationRecordsApi.uploadImage(uploadedFile.file)
          }
      ...
      const recordData = {
        id: isEditMode ? editRecordId : `meditation_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        ...
        attachments,
        tags,
        createdAt: isEditMode ? mappedMeditationRecords.find(r => r.id === editRecordId)?.createdAt.toISOString() || new Date().toISOString() : new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      }

      if (isEditMode) {
        await meditationRecordsApi.updateRecord(recordData)
        toast({
          title: "成功",
          description: "冥想记录已更新",
        })
      } else {
        await meditationRecordsApi.addRecord(recordData)
        toast({
          title: "成功",
          description: "冥想记录已保存",
        })
      ...
      toast({
        title: "保存失败",
        description: error instanceof Error ? error.message : "保存冥想记录时发生错误",
        variant: "destructive",
      })
    } finally {
      ...
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {/* 记录内容 */}
            <div className="space-y-2">
              <Label htmlFor="content">冥想记录内容</Label>
              <Textarea
                id="content"
                placeholder="请输入冥想记录内容..."
                value={content}
        ...
        <div className="flex space-x-4">
          ...
          >
            {isSubmitting ? "保存中..." : (isEditMode ? "更新冥想记录" : "保存冥想记录")}
          </Button>
        </div>
      </div>
