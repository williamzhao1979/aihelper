...
import { ArrowLeft, Upload, X, FileText, Image, File, Users, Pill } from "lucide-react"
...
import type { FileAttachment } from '@/hooks/use-medication-records'
...
export default function MedicationPage() {
  ...
  console.log("[MedicationPage] selectedUser?.uniqueOwnerId:", selectedUser?.uniqueOwnerId)
  console.log("[MedicationPage] selectedUser:", selectedUser)
  console.log("[MedicationPage] currentUser?.uniqueOwnerId:", currentUser?.uniqueOwnerId)

  const handleUserSelectionChange = (user: UserProfile) => {
    console.log('[MedicationPage] User selection changed to:', user)
    // 用户选择变化会通过全局状态自动同步
  }

  // medicationRecordsApi 必须在 selectedUser 声明后
  const medicationRecordsApi = useMedicationRecords(
    selectedUser?.ownerId || '',
    selectedUser?.uniqueOwnerId || ''
  )

  // 使用与view页面完全一致的映射逻辑
  const mappedMedicationRecords = useMemo(() => {
    console.log('[mappedMedicationRecords] useMemo triggered')
    console.log('[mappedMedicationRecords] medicationRecords length:', medicationRecordsApi.records.length)
    console.log('[mappedMedicationRecords] selectedUser:', selectedUser)
    console.log('[mappedMedicationRecords] mapping records, medicationRecords:', medicationRecordsApi.records)
    return medicationRecordsApi.records.map((r) => ({
      id: r.id,
      recordId: r.id,
  ...
  }, [medicationRecordsApi.records, selectedUser])

  // 强制获取最新数据 - 每次进入medication页面时都强制刷新云端数据（与view页面保持一致）
  useEffect(() => {
    if (!selectedUser?.uniqueOwnerId) return
    console.log('[useEffect] Medication页面强制云端刷新触发. selectedUser:', selectedUser)
    
    const doForceRefresh = async () => {
      try {
        console.log('[useEffect] Medication页面开始强制云端刷新，用户:', selectedUser?.uniqueOwnerId)
        // 使用forceRefresh确保清除所有缓存并获取最新数据
        await medicationRecordsApi.forceRefresh()
        console.log('[useEffect] Medication页面强制云端刷新完成')
      } catch (err) {
        console.error('[useEffect] Medication页面强制云端刷新失败，尝试syncFromCloud:', err)
        try {
          await medicationRecordsApi.syncFromCloud()
          console.log('[useEffect] Medication页面syncFromCloud完成')
        } catch (syncErr) {
          console.error('[useEffect] Medication页面syncFromCloud也失败:', syncErr)
        }
      }
    }
    doForceRefresh()
    ...
    if (editId) {
      ...
      const loadEditRecord = async () => {
        try {
          ...
          // 等待records数据加载完成
          if (mappedMedicationRecords.length === 0) {
            // 如果records还没加载，等待一下
            setTimeout(loadEditRecord, 100)
            return
          }
          
          const recordToEdit = mappedMedicationRecords.find(r => r.id === editId)
  ...
  }, [searchParams, mappedMedicationRecords, toast])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const uploadedFile of uploadedFiles) {
        try {
          ...
          if (uploadedFile.file) {
            fileUrl = await medicationRecordsApi.uploadImage(uploadedFile.file)
          }
      ...
      const recordData = {
        id: isEditMode ? editRecordId : `medication_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        ...
        attachments,
        tags,
        createdAt: isEditMode ? mappedMedicationRecords.find(r => r.id === editRecordId)?.createdAt.toISOString() || new Date().toISOString() : new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      }

      if (isEditMode) {
        await medicationRecordsApi.updateRecord(recordData)
        toast({
          title: "成功",
          description: "用药记录已更新",
        })
      } else {
        await medicationRecordsApi.addRecord(recordData)
        toast({
          title: "成功",
          description: "用药记录已保存",
        })
      ...
      toast({
        title: "保存失败",
        description: error instanceof Error ? error.message : "保存用药记录时发生错误",
        variant: "destructive",
      })
    } finally {
      ...
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {/* 记录内容 */}
            <div className="space-y-2">
              <Label htmlFor="content">用药记录内容</Label>
              <Textarea
                id="content"
                placeholder="请输入用药记录内容..."
                value={content}
        ...
        <div className="flex space-x-4">
          ...
          >
            {isSubmitting ? "保存中..." : (isEditMode ? "更新用药记录" : "保存用药记录")}
          </Button>
        </div>
      </div>
