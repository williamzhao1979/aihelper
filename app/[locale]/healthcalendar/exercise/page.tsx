...
import { ArrowLeft, Upload, X, FileText, Image, File, Users, Dumbbell } from "lucide-react"
...
import type { FileAttachment } from '@/hooks/use-exercise-records'
...
export default function ExercisePage() {
  ...
  console.log("[ExercisePage] selectedUser?.uniqueOwnerId:", selectedUser?.uniqueOwnerId)
  console.log("[ExercisePage] selectedUser:", selectedUser)
  console.log("[ExercisePage] currentUser?.uniqueOwnerId:", currentUser?.uniqueOwnerId)

  const handleUserSelectionChange = (user: UserProfile) => {
    console.log('[ExercisePage] User selection changed to:', user)
    // 用户选择变化会通过全局状态自动同步
  }

  // exerciseRecordsApi 必须在 selectedUser 声明后
  const exerciseRecordsApi = useExerciseRecords(
    selectedUser?.ownerId || '',
    selectedUser?.uniqueOwnerId || ''
  )

  // 使用与view页面完全一致的映射逻辑
  const mappedExerciseRecords = useMemo(() => {
    console.log('[mappedExerciseRecords] useMemo triggered')
    console.log('[mappedExerciseRecords] exerciseRecords length:', exerciseRecordsApi.records.length)
    console.log('[mappedExerciseRecords] selectedUser:', selectedUser)
    console.log('[mappedExerciseRecords] mapping records, exerciseRecords:', exerciseRecordsApi.records)
    return exerciseRecordsApi.records.map((r) => ({
      id: r.id,
      recordId: r.id,
  ...
  }, [exerciseRecordsApi.records, selectedUser])

  // 强制获取最新数据 - 每次进入exercise页面时都强制刷新云端数据（与view页面保持一致）
  useEffect(() => {
    if (!selectedUser?.uniqueOwnerId) return
    console.log('[useEffect] Exercise页面强制云端刷新触发. selectedUser:', selectedUser)
    
    const doForceRefresh = async () => {
      try {
        console.log('[useEffect] Exercise页面开始强制云端刷新，用户:', selectedUser?.uniqueOwnerId)
        // 使用forceRefresh确保清除所有缓存并获取最新数据
        await exerciseRecordsApi.forceRefresh()
        console.log('[useEffect] Exercise页面强制云端刷新完成')
      } catch (err) {
        console.error('[useEffect] Exercise页面强制云端刷新失败，尝试syncFromCloud:', err)
        try {
          await exerciseRecordsApi.syncFromCloud()
          console.log('[useEffect] Exercise页面syncFromCloud完成')
        } catch (syncErr) {
          console.error('[useEffect] Exercise页面syncFromCloud也失败:', syncErr)
        }
      }
    }
    doForceRefresh()
    ...
    if (editId) {
      ...
      const loadEditRecord = async () => {
        try {
          ...
          // 等待records数据加载完成
          if (mappedExerciseRecords.length === 0) {
            // 如果records还没加载，等待一下
            setTimeout(loadEditRecord, 100)
            return
          }
          
          const recordToEdit = mappedExerciseRecords.find(r => r.id === editId)
  ...
  }, [searchParams, mappedExerciseRecords, toast])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const uploadedFile of uploadedFiles) {
        try {
          ...
          if (uploadedFile.file) {
            fileUrl = await exerciseRecordsApi.uploadImage(uploadedFile.file)
          }
      ...
      const recordData = {
        id: isEditMode ? editRecordId : `exercise_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        ...
        attachments,
        tags,
        createdAt: isEditMode ? mappedExerciseRecords.find(r => r.id === editRecordId)?.createdAt.toISOString() || new Date().toISOString() : new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      }

      if (isEditMode) {
        await exerciseRecordsApi.updateRecord(recordData)
        toast({
          title: "成功",
          description: "运动记录已更新",
        })
      } else {
        await exerciseRecordsApi.addRecord(recordData)
        toast({
          title: "成功",
          description: "运动记录已保存",
        })
      ...
      toast({
        title: "保存失败",
        description: error instanceof Error ? error.message : "保存运动记录时发生错误",
        variant: "destructive",
      })
    } finally {
      ...
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {/* 记录内容 */}
            <div className="space-y-2">
              <Label htmlFor="content">运动记录内容</Label>
              <Textarea
                id="content"
                placeholder="请输入运动记录内容..."
                value={content}
        ...
        <div className="flex space-x-4">
          ...
          >
            {isSubmitting ? "保存中..." : (isEditMode ? "更新运动记录" : "保存运动记录")}
          </Button>
        </div>
      </div>
