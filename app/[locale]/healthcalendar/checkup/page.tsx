...
import { ArrowLeft, Upload, X, FileText, Image, File, Users, Activity } from "lucide-react"
...
import type { FileAttachment } from '@/hooks/use-checkup-records'
...
export default function CheckupRecordPage() {
  ...
  console.log("[CheckupRecordPage] selectedUser?.uniqueOwnerId:", selectedUser?.uniqueOwnerId)
  console.log("[CheckupRecordPage] selectedUser:", selectedUser)
  console.log("[CheckupRecordPage] currentUser?.uniqueOwnerId:", currentUser?.uniqueOwnerId)

  const handleUserSelectionChange = (user: UserProfile) => {
    console.log('[CheckupRecordPage] User selection changed to:', user)
    // 用户选择变化会通过全局状态自动同步
  }

  // checkupRecordsApi 必须在 selectedUser 声明后
  const checkupRecordsApi = useCheckupRecords(
    selectedUser?.ownerId || '',
    selectedUser?.uniqueOwnerId || ''
  )

  // 使用与view页面完全一致的映射逻辑
  const mappedCheckupRecords = useMemo(() => {
    console.log('[mappedCheckupRecords] useMemo triggered')
    console.log('[mappedCheckupRecords] checkupRecords length:', checkupRecordsApi.records.length)
    console.log('[mappedCheckupRecords] selectedUser:', selectedUser)
    console.log('[mappedCheckupRecords] mapping records, checkupRecords:', checkupRecordsApi.records)
    return checkupRecordsApi.records.map((r) => ({
      id: r.id,
      recordId: r.id,
  ...
  }, [checkupRecordsApi.records, selectedUser])

  // 强制获取最新数据 - 每次进入checkup页面时都强制刷新云端数据（与view页面保持一致）
  useEffect(() => {
    if (!selectedUser?.uniqueOwnerId) return
    console.log('[useEffect] CheckupRecord页面强制云端刷新触发. selectedUser:', selectedUser)
    
    const doForceRefresh = async () => {
      try {
        console.log('[useEffect] CheckupRecord页面开始强制云端刷新，用户:', selectedUser?.uniqueOwnerId)
        // 使用forceRefresh确保清除所有缓存并获取最新数据
        await checkupRecordsApi.forceRefresh()
        console.log('[useEffect] CheckupRecord页面强制云端刷新完成')
      } catch (err) {
        console.error('[useEffect] CheckupRecord页面强制云端刷新失败，尝试syncFromCloud:', err)
        try {
          await checkupRecordsApi.syncFromCloud()
          console.log('[useEffect] CheckupRecord页面syncFromCloud完成')
        } catch (syncErr) {
          console.error('[useEffect] CheckupRecord页面syncFromCloud也失败:', syncErr)
        }
      }
    }
    doForceRefresh()
  ...
  useEffect(() => {
    ...
    if (editId) {
      ...
      const loadRecordFromMappedData = async () => {
        console.log("开始从mappedCheckupRecords中查找记录:", editId)
        console.log("当前mappedCheckupRecords数量:", mappedCheckupRecords.length)
        
        // 从mappedCheckupRecords中查找记录（与view页面使用相同的数据源）
        const record = mappedCheckupRecords.find(r => r.id === editId || r.recordId === editId)
        
        if (record && record.type === "checkup") {
          console.log("从mappedCheckupRecords找到记录:", record)
          await loadRecordForEdit(editId)
        } else {
          console.log("在mappedCheckupRecords中未找到记录，等待数据加载...")
          // 如果当前没有找到记录，可能是数据还在加载中，等待一下再试
          setTimeout(() => {
            const retryRecord = mappedCheckupRecords.find(r => r.id === editId || r.recordId === editId)
            if (retryRecord && retryRecord.type === "checkup") {
  ...
  }, [searchParams, mappedCheckupRecords]) // 使用mappedCheckupRecords作为依赖项
  ...
  const loadRecordForEdit = async (recordId: string) => {
    ...
    console.log("mappedCheckupRecords数量:", mappedCheckupRecords.length)
    setIsLoadingRecord(true) // 使用专门的记录加载状态
    
    try {
      // 从mappedCheckupRecords中查找记录（与view页面使用相同的数据源）
      console.log("从mappedCheckupRecords中查找:", recordId)
      console.log("当前mappedCheckupRecords:", mappedCheckupRecords)
      
      let record = mappedCheckupRecords.find(r => r.id === recordId || r.recordId === recordId)
      
      if (record) {
        console.log("从mappedCheckupRecords找到记录:", record)
      } else {
        console.log("在mappedCheckupRecords中未找到记录")
        // 如果mappedCheckupRecords中没有找到记录，显示错误
        toast({
          title: "记录不存在",
      ...
      if (record && record.type === "checkup") {
        ...
        console.log("mappedCheckupRecords ID列表:", mappedCheckupRecords.map(r => r.id))
  ...
  const handleSubmit = async () => {
    ...
    try {
      console.log('[CheckupRecord] handleSubmit start, isEditMode:', isEditMode, 'uploadedFiles:', uploadedFiles)
      // 构造 CheckupRecord
      const newRecord = {
        ...
        type: 'checkup' as const, // 使用checkup类型
      ...
      console.log('[CheckupRecord] newRecord:', newRecord)
      console.log('[CheckupRecord] checkupRecordsApi:', checkupRecordsApi)
      
      let attachments: FileAttachment[] = []
      if (uploadedFiles.length > 0 && checkupRecordsApi) {
        for (const file of uploadedFiles) {
          console.log('[CheckupRecord] 准备上传文件:', file)
          if ((file as any).url) {
            ...
            console.log('[CheckupRecord] 已存在的附件，直接保留:', file)
          } else if (file.file) {
            try {
              const url = await checkupRecordsApi.uploadImage(file.file)
              console.log('[CheckupRecord] 上传成功，url:', url)
              attachments.push({
              ...
              console.error('[CheckupRecord] Supabase 上传失败:', e)
              toast({
                title: '文件上传失败',
      ...
      console.log('[CheckupRecord] 最终 attachments:', attachments)
      
      if (checkupRecordsApi) {
        if (isEditMode) {
          console.log('[CheckupRecord] 调用 updateRecord', newRecord)
          await checkupRecordsApi.updateRecord(newRecord)
        } else {
          console.log('[CheckupRecord] 调用 addRecord', newRecord)
          // 对于新建记录，已经在上面处理了所有文件上传，直接调用 addRecord
          await checkupRecordsApi.addRecord(newRecord)
        }
      }
      
      toast({
        title: "保存成功",
        description: `已为 ${selectedUser.nickname} 保存体检记录（含云端同步）`,
      })
      ...
      toast({
        title: "保存失败",
        description: checkupRecordsApi?.error || '保存体检记录时发生错误，请重试',
        variant: "destructive",
      })
    } finally {
      setIsSubmitting(false)
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600">加载体检记录中...</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 p-4">
          ...
          <div className="flex items-center space-x-2">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-900">
                {isEditMode ? "编辑体检记录" : "体检记录"}
              </h1>
              <p className="text-sm text-gray-600">
                {isEditMode ? "修改体检记录信息" : "记录健康体检信息"}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="max-w-2xl mx-auto space-y-6">
        ...
        <Card className="bg-white/90 backdrop-blur-sm shadow-lg">
          <CardHeader>
            <CardTitle>体检内容</CardTitle>
          </CardHeader>
          <CardContent>
            <Textarea
              placeholder="详细描述您的体检项目、结果和医生建议..."
              value={content}
          ...
          <CardContent>
            <div className="space-y-3">
              <div className="flex space-x-2">
                <Input
                  placeholder="添加标签（如：常规体检、专科检查、复查等）"
                  value={newTag}
        ...
        <div className="flex space-x-4">
          ...
          >
            {isSubmitting ? "保存中..." : (isEditMode ? "更新体检记录" : "保存体检记录")}
          </Button>
        </div>
      </div>
