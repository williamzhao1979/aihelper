...
import type { FileAttachment } from '@/hooks/use-mood-records'
...
export default function MoodPage() {
  ...
  console.log("[MoodPage] selectedUser?.uniqueOwnerId:", selectedUser?.uniqueOwnerId)
  console.log("[MoodPage] selectedUser:", selectedUser)
  console.log("[MoodPage] currentUser?.uniqueOwnerId:", currentUser?.uniqueOwnerId)

  const handleUserSelectionChange = (user: UserProfile) => {
    console.log('[MoodPage] User selection changed to:', user)
    // 用户选择变化会通过全局状态自动同步
  }

  // moodRecordsApi 必须在 selectedUser 声明后
  const moodRecordsApi = useMoodRecords(
    selectedUser?.ownerId || '',
    selectedUser?.uniqueOwnerId || ''
  )

  // 使用与view页面完全一致的映射逻辑
  const mappedMoodRecords = useMemo(() => {
    console.log('[mappedMoodRecords] useMemo triggered')
    console.log('[mappedMoodRecords] moodRecords length:', moodRecordsApi.records.length)
    console.log('[mappedMoodRecords] selectedUser:', selectedUser)
    console.log('[mappedMoodRecords] mapping records, moodRecords:', moodRecordsApi.records)
    return moodRecordsApi.records.map((r) => ({
      id: r.id,
      recordId: r.id,
  ...
  }, [moodRecordsApi.records, selectedUser])
    ...
    if (editId) {
      ...
      const loadEditRecord = async () => {
        try {
          ...
          // 等待records数据加载完成
          if (mappedMoodRecords.length === 0) {
            // 如果records还没加载，等待一下
            setTimeout(loadEditRecord, 100)
            return
          }
          
          const recordToEdit = mappedMoodRecords.find(r => r.id === editId)
  ...
  }, [searchParams, mappedMoodRecords, toast])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const uploadedFile of uploadedFiles) {
        try {
          ...
          if (uploadedFile.file) {
            fileUrl = await moodRecordsApi.uploadImage(uploadedFile.file)
          }
      ...
      const recordData = {
        id: isEditMode ? editRecordId : `mood_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        ...
        attachments,
        tags,
        createdAt: isEditMode ? mappedMoodRecords.find(r => r.id === editRecordId)?.createdAt.toISOString() || new Date().toISOString() : new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      }

      if (isEditMode) {
        await moodRecordsApi.updateRecord(recordData)
        toast({
          title: "成功",
          description: "心情记录已更新",
        })
      } else {
        await moodRecordsApi.addRecord(recordData)
        toast({
          title: "成功",
          description: "心情记录已保存",
        })
      ...
      toast({
        title: "保存失败",
        description: error instanceof Error ? error.message : "保存心情记录时发生错误",
        variant: "destructive",
      })
    } finally {
      ...
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {/* 记录内容 */}
            <div className="space-y-2">
              <Label htmlFor="content">心情记录内容</Label>
              <Textarea
                id="content"
                placeholder="请输入心情记录内容..."
                value={content}
        ...
        <div className="flex space-x-4">
          ...
          >
            {isSubmitting ? "保存中..." : (isEditMode ? "更新心情记录" : "保存心情记录")}
          </Button>
        </div>
      </div>
