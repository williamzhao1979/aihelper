...
import type { FileAttachment } from '@/hooks/use-thoughts-records'
...
export default function ThoughtsPage() {
  ...
  console.log("[ThoughtsPage] selectedUser?.uniqueOwnerId:", selectedUser?.uniqueOwnerId)
  console.log("[ThoughtsPage] selectedUser:", selectedUser)
  console.log("[ThoughtsPage] currentUser?.uniqueOwnerId:", currentUser?.uniqueOwnerId)

  const handleUserSelectionChange = (user: UserProfile) => {
    console.log('[ThoughtsPage] User selection changed to:', user)
    // 用户选择变化会通过全局状态自动同步
  }

  // thoughtRecordsApi 必须在 selectedUser 声明后
  const thoughtRecordsApi = useThoughtRecords(
    selectedUser?.ownerId || '',
    selectedUser?.uniqueOwnerId || ''
  )

  // 使用与view页面完全一致的映射逻辑
  const mappedThoughtRecords = useMemo(() => {
    console.log('[mappedThoughtRecords] useMemo triggered')
    console.log('[mappedThoughtRecords] thoughtRecords length:', thoughtRecordsApi.records.length)
    console.log('[mappedThoughtRecords] selectedUser:', selectedUser)
    console.log('[mappedThoughtRecords] mapping records, thoughtRecords:', thoughtRecordsApi.records)
    return thoughtRecordsApi.records.map((r) => ({
      id: r.id,
      recordId: r.id,
  ...
  }, [thoughtRecordsApi.records, selectedUser])

  // 强制获取最新数据 - 每次进入thought页面时都强制刷新云端数据（与view页面保持一致）
  useEffect(() => {
    if (!selectedUser?.uniqueOwnerId) return
    console.log('[useEffect] Thought页面强制云端刷新触发. selectedUser:', selectedUser)
    
    const doForceRefresh = async () => {
      try {
        console.log('[useEffect] Thought页面开始强制云端刷新，用户:', selectedUser?.uniqueOwnerId)
        // 使用forceRefresh确保清除所有缓存并获取最新数据
        await thoughtRecordsApi.forceRefresh()
        console.log('[useEffect] Thought页面强制云端刷新完成')
      } catch (err) {
        console.error('[useEffect] Thought页面强制云端刷新失败，尝试syncFromCloud:', err)
        try {
          await thoughtRecordsApi.syncFromCloud()
          console.log('[useEffect] Thought页面syncFromCloud完成')
        } catch (syncErr) {
          console.error('[useEffect] Thought页面syncFromCloud也失败:', syncErr)
        }
      }
    }
    doForceRefresh()
    ...
    if (editId) {
      ...
      const loadEditRecord = async () => {
        try {
          ...
          // 等待records数据加载完成
          if (mappedThoughtRecords.length === 0) {
            // 如果records还没加载，等待一下
            setTimeout(loadEditRecord, 100)
            return
          }
          
          const recordToEdit = mappedThoughtRecords.find(r => r.id === editId)
  ...
  }, [searchParams, mappedThoughtRecords, toast])
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      for (const uploadedFile of uploadedFiles) {
        try {
          ...
          if (uploadedFile.file) {
            fileUrl = await thoughtRecordsApi.uploadImage(uploadedFile.file)
          }
      ...
      const recordData = {
        id: isEditMode ? editRecordId : `thought_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        ...
        attachments,
        tags,
        createdAt: isEditMode ? mappedThoughtRecords.find(r => r.id === editRecordId)?.createdAt.toISOString() || new Date().toISOString() : new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      }

      if (isEditMode) {
        await thoughtRecordsApi.updateRecord(recordData)
        toast({
          title: "成功",
          description: "想法记录已更新",
        })
      } else {
        await thoughtRecordsApi.addRecord(recordData)
        toast({
          title: "成功",
          description: "想法记录已保存",
        })
      ...
      console.error("保存想法记录失败:", error)
      toast({
        title: "保存失败",
        description: error instanceof Error ? error.message : "保存想法记录时发生错误",
        variant: "destructive",
      })
    } finally {
      ...
      <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {/* 记录内容 */}
            <div className="space-y-2">
              <Label htmlFor="content">想法记录内容</Label>
              <Textarea
                id="content"
                placeholder="请输入想法记录内容..."
                value={content}
        ...
        <div className="flex space-x-4">
          ...
          >
            {isSubmitting ? "保存中..." : (isEditMode ? "更新想法记录" : "保存想法记录")}
          </Button>
        </div>
      </div>
