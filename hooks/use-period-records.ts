...
export interface PeriodRecord {
  id: string;
  date: string;
  datetime?: string; // 添加datetime字段，对应生理记录页面的日期时间
  type: 'period';
  content: string;
...
interface UsePeriodRecordsResult {
  records: PeriodRecord[];
  addRecord: (record: PeriodRecord, imageFile?: File) => Promise<void>;
  updateRecord: (record: PeriodRecord) => Promise<void>;
...
const LOCAL_KEY_PREFIX = 'healthcalendar-period-records-';
...
export function usePeriodRecords(currentUserId: string, uniqueOwnerId: string): UsePeriodRecordsResult {
  ...
  const uploadImage = useCallback(async (file: File) => {
    setLoading(true);
    setError(null);
    const filePath = `users/${uniqueOwnerId}/period_attachments/${Date.now()}_${file.name}`;
  ...
  const saveLocal = useCallback((newRecords: PeriodRecord[]) => {
  ...
  const syncFromCloud = useCallback(async () => {
    ...
    try {
      ...
      setError('Failed to parse period-records.json');
    }
    setLoading(false);
  }, [uniqueOwnerId, saveLocal]);

  // 新增记录
  const addRecord = useCallback(async (record: PeriodRecord, imageFile?: File) => {
    setLoading(true);
    setError(null);
  ...
  // 更新记录
  const updateRecord = useCallback(async (record: PeriodRecord) => {
    setLoading(true);
    setError(null);
